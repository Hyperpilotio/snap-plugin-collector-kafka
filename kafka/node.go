...
type nodeTarget struct {
	// URI the target URI such as  java.lang:type=MemoryPool,name=G1 Old Gen
	URI    string
	Loaded bool
}
...
func (n *node) addXMLAttributes(ns string, attrs []XMLAttribute) {
...
func (n *node) loadElements(url string) error {
	...
	attrs, err := getResp(url, n.Target.URI)
	if err != nil {
	...
	n.addXMLAttributes(strings.Join(ns, "/"), attrs)
...
func getResp(url, uri string) ([]XMLAttribute, error) {
	...
	attrs, err := readXMLAttributes(contents)
	return attrs, err
}
